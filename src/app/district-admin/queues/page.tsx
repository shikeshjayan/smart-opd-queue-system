...
export default function DistrictQueuesPage() {
  ...
  const filtersReady = districtId !== null;
  ...
  }, [data, hospitalFilter, departmentFilter, statusFilter, minWaiting]);
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Queue Monitor"
        description="Live queue position for every OPD in the district."
        actions={<LiveIndicator />}
      />

      <div className="grid grid-cols-1 gap-4 rounded-card border border-ink-200 bg-surface p-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <QueueOverview items={filtered} scope="district" />
    </div>
  );
}
