...
function DoctorNotificationsContent() {
  ...
  return (
    <div className="max-w-4xl">
      <PageHeader
        title="Notifications"
        description="Lab verifications, critical alerts, and system updates."
        actions={
          <NotificationBell />
        }
      />
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h3 className="font-medium text-ink-900">
            Your Notifications {unread > 0 && <span className="ml-2 rounded-full bg-brand-600 px-2 py-0.5 text-xs font-medium text-white">{unread}</span>}
          </h3>
          {unread > 0 && (
